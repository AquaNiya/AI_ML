...
while True:
  ...
  base_bill = input("> $ ")
  #check if it can can be converted to a float
  try:
    float(base_bill)
    #check if it's not a negative no.
    #zero is acceptable
    if float(base_bill) < 0:
      print("Bill can't be negative. Please enter a valid amout.")
    else:
      break
  #if not ask again
  except ValueError:
    print("Please enter a valid amout.")
